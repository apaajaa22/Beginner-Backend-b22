    ...
    if (!err) {
      categoryModel.getCountCategory((err, resultCount, _fields) => {
        if (!err) {
          ...
          pageInfo.nextPage =
            data.page < lastPage
              ? `${APP_URL}/categories?page=${data.page + 1}`
              : null
          pageInfo.prevPage =
            data.page > 1
              ? `${APP_URL}/categories?page=${data.page - 1}`
...
exports.getProductsByCategory = (req, res) => {
  ...
  getProductsByCategory(id, data, (err, results, _fields) => {
    if (!err) {
      ...
      categoryModel.getCountProductByCategory(
        id,
        (err, resultCount, _fields) => {
          if (!err) {
            const totalData = resultCount[0].count
            const lastPage = Math.ceil(totalData / data.limit)
            pageInfo.totalData = totalData
            ...
            pageInfo.nextPage =
              data.page < lastPage
                ? `${APP_URL}/categories/${id}/products?page=${data.page + 1}`
                : null
            pageInfo.prevPage =
              data.page > 1
                ? `${APP_URL}/categories/${id}/products?page=${data.page - 1}`
      ...
      if (!err) {
        if (results.length > 0) {
          ...
          if (key.length > 1) {
            ...
            categoryModel.updateCategoryPartial(
              updateData,
              (err, results, _fields) => {
                if (!err) {
                  return standardResponse(
