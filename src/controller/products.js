...
exports.createProduct = async (req, res) => {
      ...
      if (!err) {
        validateInteger(res, req.body.price, 'Price', () => {
          validateInteger(res, req.body.quantity, 'Quantity', () => {
            req.body.picture = req.file ? `${APP_UPLOAD_ROUTE}/${req.file.filename}` : null
            productModel.createProduct(req.body, (err, results, _fields) => {
              if (!err) {
                if (results.affectedRows > 0) {
                  ...
                  return standardResponse(res, 'Product created successfully', null, 200)
                } else {
  ...
  //           validateInteger(res, req.body.price, 'Price', () => {
  //             validateInteger(res, req.body.quantity, 'Quantity', () => {
  //               req.body.picture = path.join(`${APP_UPLOAD_ROUTE}/${req.file.filename}`)
  //               productModel.createProduct(req.body, (err, results, _fields) => {
  //                 if (!err) {
  //                   if (results.affectedRows > 0) {
  //                     if (typeof req.body.category !== 'object') {
  ...
  //     }
  //   } else {
  //     return standardResponse(res, 'You must log in first', null, 401)
  //   }
  // })
}
...
exports.getProducts = (req, res) => {
  ...
  const pageInfo = {}
  if (search && sort) {
    productModel.sortProducts(search, sort, data, order, (err, results, _fields) => {
      if (!err) {
        productModel.getProductsCount(data, (err, resultCount, _fields) => {
          if (!err) {
            ...
            return standardResponse(res, 'product found', results, 200, pageInfo)
          } else {
      ...
      if (!err) {
        productModel.getCount((err, resultCount, _fields) => {
          if (!err) {
            ...
            pageInfo.nextPage = data.page < lastPage ? `${APP_URL}/products?page=${data.page + 1}` : null
            pageInfo.prevPage = data.page > 1 ? `${APP_URL}/products?page=${data.page - 1}` : null
            return standardResponse(res, 'product sorted', results, 200, pageInfo)
          } else {
            return standardResponse(res, 'product not found', null, 404, pageInfo)
          }
        })
      ...
      if (!err) {
        productModel.getCount((err, resultCount, _fields) => {
          if (!err) {
            ...
            return standardResponse(res, 'product not found', results, 404, pageInfo)
          }
        })
      }
    })
  ...
  productModel.getProductDetails(id, (err, results, _fields) => {
    if (!err) {
      if (results.length > 0) {
        ...
        const hiddenColumn = ['additional_price', 'end_price', 'variant', 'variant_code']
      ...
      if (!err) {
        if (results.length > 0) {
          ...
          if (key.length > 1) {
            ...
            productModel.updateProductPartial(updateData, (err, results, _fields) => {
              if (!err) {
                return standardResponse(res, 'Product updated successfully', null, 200)
              } else {
                return standardResponse(res, 'Product failed to update', null, 400)
              }
            })
          }
        } else {
      ...
      if (!err) {
        if (results.length > 0) {
          ...
          productModel.updateProduct(updateData, (err, results, _fields) => {
            if (!err) {
              return standardResponse(res, 'Item updated successfully', null, 200)
            } else {
              return standardResponse(res, 'An error occurred', null, 400)
            }
          })
      ...
      if (!err) {
        if (results.length > 0) {
          productModel.deleteProduct(id, (err, results, _fields) => {
            if (!err) {
              return standardResponse(res, 'product has been deleted!', null, 200)
            } else {
              return standardResponse(res, 'product not found!', null, 404)
            }
          })
